...
#https://github.com/rahmanidashti/LBSNDatasets/tree/master/Yelp/Yelp_4
cities="Yelp"

# This cities are for the original dataset of Foursquare
KCore=2
coordFile="Yelp_poi_coos.txt"
...
do
  ...
  downloadedfile=$processedCities/Yelp_check_ins.txt
  filecorrectformat=$processedCities/Yelp_check_ins_format.txt
  awk 'BEGIN {OFS="\t"} {print $1, $2, 1, int($3)}' $downloadedfile > $filecorrectformat


  #KCore
  fileNewIds=$filecorrectformat
  ...
  if [ ! -f "$fileStatistics" ]; then
    $javaCommand $jvmMemory -jar $JAR -o SatatisticsPercentageCheckinsByMidPoint -trf $trainFile -coordFile "$processedCities"/$coordFile -orf $fileStatistics -listNumbers "1,5,10,15,20,2000"
  fi
...
do
  ...
  do
    ...
    do
      ...
      $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr RandomRecommender -orf $outputImputationFile -perUser $perUser -perIncr $perIncr -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -n $neighbours -reverse $reverse
      ...
      $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr AverageDistanceUserGEO -rs "notUsed" -orf $outputImputationFile -perUser $perUser -perIncr $perIncr -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -n $neighbours -scoreFreq FREQUENCY -reverse $reverse
      ...
      $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr ItemNeighborhoodRecommender -rs $IBsimilarity -n $neighbours -orf $outputImputationFile -perUser $perUser -perIncr $perIncr -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -reverse $reverse
      ...
      $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr PopGeoNN -rs $UBsimilarity -n $neighbours -orf $outputImputationFile -perUser $perUser -perIncr $perIncr -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -reverse $reverse
  ...
  do
    ...
    $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr RandomRecommender -orf $outputImputationFile -perUser $perUser -perIncr $perIncr2 -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -n $neighbours -reverse true -imputeAllUsers true
    ...
    $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr ItemNeighborhoodRecommender -rs $IBsimilarity -n $neighbours -orf $outputImputationFile -perUser $perUser -perIncr $perIncr2 -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -reverse true -imputeAllUsers true
    ...
    $javaCommand $jvmMemory -jar $JAR -o ImputationDataset -trf $trainfile -tsf $testfile -cIndex false -rr PopGeoNN -rs $UBsimilarity -n $neighbours -orf $outputImputationFile -perUser $perUser -perIncr $perIncr2 -coordFile "$processedCities"/$coordFile -thr $distanceMidpoint -reverse true -imputeAllUsers true
